...
body_h =
lock_neck_h + neck_transition_h + funnel_h + bin_h;
...
socket_inner_d =
lock_neck_od + 2 * lock_clearance;

socket_outer_d =
lock_neck_od + 2 * (lock_tab_d + socket_wall);

// Entry slot width
slot_w =
lock_tab_w + 2 * lock_clearance;

// Radial depth required for tabs
slot_radial =
lock_tab_d + lock_clearance + 0.8;

// Vertical locking groove
groove_h =
lock_tab_h + 2 * lock_z_clearance;

// Angular allowance for physical width of each tab
tab_half_angle =
atan(
  (slot_w / 2) / (lock_neck_od / 2 + lock_tab_d / 2)
);
...
cap_inner_x =
top_x + 2 * cap_clearance;

cap_inner_y =
top_y + 2 * cap_clearance;

cap_outer_x =
cap_inner_x + 2 * cap_wall;

cap_outer_y =
cap_inner_y + 2 * cap_wall;
